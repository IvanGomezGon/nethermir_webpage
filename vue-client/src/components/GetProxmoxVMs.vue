  ...
  export default {
    ...
    methods: {
        getData(){
            ...
            fetch(`http://nethermir.uab.cat:80/backend/getNodes?server=${this.server}`, {credentials: "include"}).then(resolve)
            })
            p.then(response=>{
        ...
        activateVM(id){
            fetch(`http://nethermir.uab.cat:80/backend/activateMachine?id=${id}&hours=4`, {credentials: "include"}).then()
        },
        stopVM(id){
            fetch(`http://nethermir.uab.cat:80/backend/stopMachine?id=${id}`, {credentials: "include"}).then()
        },
        resumeVM(id){
            console.log("yay")
            fetch(`http://nethermir.uab.cat:80/backend/resumeMachine?id=${id}`, {credentials: "include"}).then()
        },
        suspendVM(id){
            fetch(`http://nethermir.uab.cat:80/backend/suspendMachine?id=${id}`, {credentials: "include"}).then()
        }
    },
  }
  </script>
